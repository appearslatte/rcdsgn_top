...
class LibForm
{
	public $formDatas;
	public $formGroups;
	public $valuestr_separator;
	public $sendTime;
	private $mail;
	
	function __construct($form_datas, $form_groups, $valuestr_separator = ","){
		$this->formDatas = $this->arrayToObject($form_datas);
		$this->formGroups = $this->arrayToObject($form_groups);
		$this->valuestr_separator = $valuestr_separator;
		$this->sendTime = "";
		$this->mail = null;
		...
		foreach($this->formDatas as $key => $val) {
			...
			if(isset($val->check_vals)) {
				...
				$val->valuestr = implode($this->valuestr_separator, $val->value);
			}
	...
	function initMail($smtpHost = "", $smtpUser = "", $smtpPass = "", $smtpSecure = "tls", $smtpPort = 587) {
	...
	function isSessionStarted() {
		return (isset($_SESSION['lform_name']) && $_SESSION['lform_name'] == $_SERVER['SCRIPT_NAME']);
	}
	...
	function csrfValidate() {
		...
		return $this->csrfGenerate() === $_POST['token'];
	}
}
